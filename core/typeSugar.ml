...
sig
  ...
  val handle_deep_resumption_effects : griper
  val resumption_type_annotation : griper
  val handle_shallow_resumption_codomain : griper
  val handle_shallow_resumption_effects : griper
  ...
  = struct
    ...
    let handle_deep_resumption_effects ~pos ~t1:(resume,lt) ~t2:(_,rt) ~error:_ =
      build_tyvar_names [lt;rt];
    ...
    let handle_shallow_resumption_codomain ~pos ~t1:(resume,lt) ~t2:(_,rt) ~error:_ =
      build_tyvar_names [lt;rt];
      die pos ("\
    ...
    let handle_shallow_resumption_effects ~pos ~t1:(resume,lt) ~t2:(_,rt) ~error:_ =
      build_tyvar_names [lt;rt];
  ...
  let rec type_pattern {node = pattern; pos = pos'} : Pattern.with_pos * Types.environment * (Types.datatype * Types.datatype) =
    ...
    let (p, env, (outer_type, inner_type)) :
      ...
      | Operation { label; parameters; resumption } ->
         let resumption, renv, codomain = match resumption with
           | None -> None, Env.empty, None
           | Some (resumption, _) ->
              let r, env, ty = type_resumption_pattern resumption in
              Some (r, ty), env, Some (TypeUtils.arg_types ty)
         in
         let parameters = List.map tp parameters in
         let effect_type typ =
           (* If our shallow resumption handler r has type `r : B -> C',
              and parameters p have type `p : A',
              then we're an effect of C = { L : A -> B | R }. *)
           let domain = List.map typ parameters in
           let codomain = match codomain with
             | None -> Types.fresh_type_variable (lin_unl, res_any)
             | Some ty -> ListUtils.last ty
           in
           let t = Types.make_function_type domain (Types.make_empty_closed_row ()) codomain in
           `Effect (make_singleton_row (label, `Present t))
         in
         let env = (List.fold_right (env ->- (++)) parameters Env.empty ++ renv) in
         Operation {
             label; parameters = List.map erase parameters; resumption
           }, env, (effect_type ot, effect_type it)
      ...
      match (expr : phrasenode) with
        ...
        | Handle { expressions; cases; descriptor } ->
           ...
           let type_effect_cases context arity cases =
             ...
             let classify i pat =
               let matrix =
                 if is_operation_pattern (fst3 pat) then
                   (* return x -> M, thus x : A *)
                   (unify ~handle:Gripers.handle_operation_patterns (no_pos operation_types.(i), ppos_and_typ pat); effect_matrix)
                 else
                   (* L_i (p : A_i) r, thus { L_i:A_i -> _ | _} ∈ E *)
                   (unify ~handle:Gripers.handle_value_patterns (no_pos value_types.(i), ppos_and_typ pat); value_matrix)
               in
             ...
             let cases = List.fold_left
                 (fun cases (patterns, resumption, body) ->
                   ...
                   (* Ensure all bodies have a consistent type (N_i : D) *)
                   unify ~handle:Gripers.handle_branches (no_pos body_type, pos_and_typ body);

                   (* Deep resumptions return the same type as the body (r : B_i -> D) *)
                   (match resumption with
                    | None -> ()
                    | Some resumption ->
                       let rp, rty = ppos_and_typ resumption in
                       unify ~handle:Gripers.handle_deep_resumption_codomain (pos_and_typ body, (rp, TypeUtils.return_type rty)));
             ...
             body_type, cases, value_types, operation_types
           in
           let (expressions, expr_eff) = type_expressions context expressions in
           let body_type, cases, value_types, operation_types = type_effect_cases context arity cases in

           (* TODO: Check griper orientation. *)
           (* TODO: Check effects of all resumption functions? Or are these implicit already??? *)

           (* Ensure the cases on the expressions' result match up (C = A!_). *)
           List.iteri (fun i (expression, _) ->
               unify ~handle:Gripers.handle_return (pos_and_typ expression, no_pos value_types.(i))) expressions;

           (* Ensure the cases on the expressions' effects match up (C = _!E). *)
           Array.iter (fun eff ->
               unify ~handle:Gripers.handle_comp_effects (no_pos (`Effect expr_eff), no_pos eff)) operation_types;

           (* Ensure each shallow resumption is well formed (r : _ -> C) *)
           List.iter (fun (patterns, _, _) ->
               List.iteri (fun i (pattern, _, _) ->
                   match pattern.node with
                   | Pattern.Operation { resumption = Some (rp, rty); _ } ->
                      let rp = uexp_pos rp in
                      let expr, _ = List.nth expressions i in
                      unify ~handle:Gripers.handle_shallow_resumption_codomain
                        ((rp, TypeUtils.return_type rty), pos_and_typ expr);
                      unify ~handle:Gripers.handle_shallow_resumption_effects
                        ((rp, `Effect (TypeUtils.effect_row rty)),
                         no_pos (`Effect expr_eff))
                   | _ -> ())
                 patterns) cases;

           (* TODO: Add the outer_effect in here too. Still not 100% sure about this, so work it out! *)
           let expr_eff = make_operations_presence_polymorphic expr_eff in
           unify ~handle:Gripers.handle_unify_with_context (no_pos (`Effect expr_eff),
                                                            no_pos (`Effect context.effect_row));

           (Handle {
                expressions = List.map (fst ->- erase) expressions;
                cases =
                  List.map
                    (fun (p, r, b) -> {
                         patterns = List.map erase p;
                         resumption = opt_map (fun (p, _, t) -> (p, t)) r;
                         body = erase b;
                    })
                    cases;
                descriptor
            ...
            body_type,
            merge_usages [
                usage_compat (List.map (fst ->- usages) expressions);
                usage_compat (List.map (thd3 ->- usages) cases);
           ])
