         ...
         begin
              ...
              if tidy then
                begin
                  if Linearity.is_nonlinear lin then
                    if Types.Unl.can_type_be t2 then
                      Types.Unl.make_type t2
                    else
                  ...
                  begin
                    ...
                    | Some const ->
                       ...
                       if M.can_type_be t2 then
                         M.make_type t2
                       else
                         let message = Printf.sprintf "Cannot unify the %s type variable %d with the non-%s type %s."
                                         (Restriction.to_string rest) var (Restriction.to_string rest) (string_of_datatype t2)
                         in
                         raise (Failure (`Msg message))
                  end;
           ...
           | _, `Var (var, (lin, rest), `Flexible) ->
              ...
              if tidy then
                begin
                  if Linearity.is_nonlinear lin then
                    if Types.Unl.can_type_be t1 then
                      Types.Unl.make_type t1
                    else
                      raise (Failure (`Msg ("Cannot unify the unlimited type variable " ^ string_of_int var ^
                  ...
                  begin
                    ...
                    | Some const ->
                       let module M = (val const) in
                       if M.can_type_be t1 then
                         M.make_type t1
                       else
                         let message = Printf.sprintf "Cannot unify the %s type variable %d with the non-%s type %s."
                                         (Restriction.to_string rest) var (Restriction.to_string rest) (string_of_datatype t1)
                         in raise (Failure (`Msg message))
                  end;
            ...
            if var_is_free_in_type var t then
              ...
              (Debug.if_set (show_recursion) (fun () -> "non-rec intro (" ^ string_of_int var ^ ")");
               ...
               begin
                 ...
                 | Some const ->
                    let module M = (val const) in
                    if M.can_type_be t then
                      M.make_type t
                    else
                      let message = Printf.sprintf "Cannot unify the %s type variable %d with the non-%s type %s."
                                      (Restriction.to_string rest) var (Restriction.to_string rest) (string_of_datatype t)
                      in
                      raise (Failure (`Msg message))
               end;
